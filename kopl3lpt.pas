{ Kefrens Bar+ with 3 dithered RGB copper bars using EGA64 lowres }
{ * Stable 60 Hz on 286/12 MHz (0WS) }
{ * Monitor is required to support 64 colors on 15 kHz modes }
{ * Plays music on OPL3LPT }
...
begin
  ...
  begin
    repeat
      ...
      port[$37a] := 1;
      port[$37a] := 5;
      {for a := 0 to adlib_delays[0] do b := port[$378];}

      port[$378] := mem[seg(imf^):imfpos+1];
      port[$37a] := 12;
      port[$37a] := 8;
      port[$37a] := 12;
      {for a := 0 to adlib_delays[1] do b := port[$378];}
...
begin
  ...
  for i := 0 to 255 do begin
    xtab[i] := 213 + round(40*sin(2*pi*i/256));
    ytab[i] := 213 + round(40*cos(2*pi*i/256));
    ztab[i] := 213 - round(40*sin(2*pi*i/256));
    write('.');
  end;
